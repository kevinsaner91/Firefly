...
% -------------------------------------------------------- %
function [best]=fa_ndim_2Point(instr)
...
%lambda= 0.5;
%a = x *bsxfun( @times, ns, covariance );
%b = bsxfun(@times, x, yield);
%z = squeeze( sum( sum( a ) ) )+(1-lambda)*-sum(b);
%------------- Portfolio Optimization Problem --------%%%
...
alpha=para(3); betamin=para(4); gamma=para(5); hybrid=para(6); divider=para(7); prob=para(8);
...
if hybrid
    pr=randi([0 1],1,1);
    if(k>MaxGeneration-(MaxGeneration/divider)) 
        for i=1:n,
            zn(i)=fhandle(ns(i,:));
            Lightn(i)=zn(i);
        end
...
function [ns]=hybridization(ns,zn,n) 

p1=randi([1 n],1,1); 
p2=randi([1 7],1,1); % randomized crossoverindex
p3=randi([7 15],1,1);
parent1=ns(1,:); % best solution
parent2=ns(p1,:);% random one
ns(n,:)=twoPointCrossing(parent1,parent2,p2,p3);

function mutant=twoPointCrossing(parent1, parent2, CrossOverIndex1, CrossOverIndex2)
    mutant = [parent1(1:CrossOverIndex1) parent2(CrossOverIndex1+1:CrossOverIndex2) parent1(CrossOverIndex2+1:end)];
