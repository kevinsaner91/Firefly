...
n=instr(1);  MaxGeneration=instr(2); hybrid=instr(3); divider=instr(4);
% Show info
%help firefly_simple.m
%rand('state',0);  % Reset the random generator

% ------ Rosenbrock function ---------------------
%str1='(1-x).^2 - 100*(y-x.^2).^2';
%str2='';

% ------ Ackley function -------------------------
str1='20*exp(-0.2*sqrt(0.5*(x^2 +y^2)))';
str2='-exp(0.5*(cos(2*pi*x)+cos(2*pi*x)))+exp(1)+20';
...
delta=0.95;      % Randomness reduction (similar to 
...
if hybrid
    ...
    if pr >= 0
        ...
        xn=xn(Index); yn=yn(Index);
        xo=xn;   yo=yn;    Lighto=Lightn;
        if(i>MaxGeneration-(MaxGeneration/divider))
            p=randi([1 n],1,1);
            parent1=[xo(n)   yo(n)];
            parent2=[xo(p)   yo(p)];
            mutant=singlePointCrossover(parent1, parent2, 1);
            xo(1)=mutant(1);
            yo(1)=mutant(2);
        end
    end
...
function [spco]=singlePointCrossover(parent1, parent2, CrossOverIndex)
    spco = [parent1(1:CrossOverIndex) parent2(CrossOverIndex+1:end)];
